...
import numpy
import os
import autumn_tool_kit as tool_kit
''' static functions '''
...
def read_input_data_xls(from_test, sheets_to_read, country, gui_console_fn):
    ...
    for sheet_name in final_sheets_to_read:
        if sheet_name == 'bcg_2016' and country == 'Philippines':
            country_to_read = 'Philippines (the)'
        else:
            country_to_read = country

        sheet_reader = SpreadsheetReader(country_to_read, sheet_name, from_test, gui_console_fn)
...
class SpreadsheetReader:
    ...
    def __init__(self, country_to_read, purpose, from_test, gui_console_fn):
        ...
        self.filename = os.path.join('data/', filename) if from_test else filename  # if from directory above
